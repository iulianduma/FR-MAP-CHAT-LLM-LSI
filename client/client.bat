...
def call_gemini(messages_history, trigger_msg=None):
    ...
    try:
        ...
        # VERIFICARE ROBUSTĂ ÎNAINTE DE A ACCESA TEXTUL
        if response.candidates and response.candidates[0].content:
            return response.text.strip()
        else:
            print(f"ATENȚIE: Răspunsul a fost blocat sau nu conține text valid. Finish Reason: {response.candidates[0].finish_reason.name if response.candidates else 'N/A'}")
            return "PASS"
